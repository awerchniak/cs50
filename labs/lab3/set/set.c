...
typedef struct set{
	entry_t *head;
	void (*print_item)(char *key, void *data);
...
bool set_insert(set_t *set, char *key, void *data)
{
	...
	} else if (key == NULL || data == NULL){			//make sure data has a value
		fprintf(stderr, "Error! Cannot link NULL data or key.\n");
		return(false);									//	return false if not
...
void print_set(set_t *set)
{
	...
	while(ptr != NULL){									//iterate through set and use user's
		(*set->print_item)(ptr->key, ptr->data);		//	function to print each element
		ptr = ptr->next;
	}
}